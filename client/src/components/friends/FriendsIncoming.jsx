...
function FriendsIncoming () {
    const { store } = useContext(Context);

    if (store.requestsIncoming.length > 0) {
        return store.requestsIncoming.map(requests => {
            for (let i = 0; i < store.users.length; i++) {
                if (requests.idUserFrom === store.users[i].id) {
                    return (
...
export default observer(FriendsIncoming);