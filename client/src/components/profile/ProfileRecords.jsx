  ...
  return (
      <div className='profile_records'>
        ...
        {store.user.avatar ? <img src={store.API_URL_UPLOADS + store.user.email.split('@')[0] + "/avatar/" + store.user.avatar} alt="profile-img" /> : <span className='profile_records-no-avatar'></span>}
          <textarea id='record-add-input' placeholder={store.user.id === store.userProfile.id ? 'Что у вас нового?' : 'Напишите что-нибудь...'} ref={inputRef} value={msg} onInput={(e) => {auto_grow(e.target); }} onClick={(e) => {setSelectStart(e.target.selectionStart); setSelectEnd(e.target.selectionEnd)}} onChange={(e) => { setMsg(e.target.value); setSelectStart(e.target.selectionStart); setSelectEnd(e.target.selectionEnd)}}></textarea>
          <button className='record-add-btn' onClick={() => {store.addRecord(store.userProfile.id, store.user.id, date, msg); setMsg('')}}>Опубликовать</button>
